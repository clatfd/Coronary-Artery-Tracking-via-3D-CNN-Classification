...
if __name__ == '__main__':
    ...
    taskname = 'CNNTracker2-1'
    if not os.path.exists(taskdir + '/' + taskname):
        os.mkdir(taskdir + '/' + taskname)
    pj_prefix = taskdir + '/' + taskname

    test_id = 6
    train_list = [str(i) for i in range(8) if i!=test_id]
    val_list = [str(test_id)]
    test_list = [str(test_id)]

    list_dir = r'D:\LiChen\RotterdanCoronary\patch/offset\point_500_gp_1'
    train_patch_list = []
    val_patch_list = []
    for pi in train_list:
        if os.path.exists(list_dir + '/d' + pi + '_patch_info_500.csv'):
            train_patch_list.append(list_dir + '/d' + pi + '_patch_info_500.csv')
    for pi in val_list:
        if os.path.exists(list_dir + '/d' + pi + '_patch_info_500.csv'):
            val_patch_list.append(list_dir + '/d' + pi + '_patch_info_500.csv')
