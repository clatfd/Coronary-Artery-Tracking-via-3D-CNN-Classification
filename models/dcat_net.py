...
class DCATNet(nn.Module):
    def __init__(self, n_classes=100):
        super(DCATNet, self).__init__()
        ...
        self.layer8 = conv_block(32, 64, 3, stride=1, p_size=0)
        self.layer9 = conv_block(64, 64, 1, stride=1, p_size=0)
        self.layer10 = nn.Conv3d(64, 1, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        out = self.layer2(out)
        out = self.layer3(out)
        out_common = self.layer4(out)
        out_1 = self.layer5(out_common)
        out_1 = self.layer6(out_1)
        out_1 = self.layer7(out_1)

        out_2 = self.layer8(out_common)
        out_2 = self.layer9(out_2)
        out_2 = self.layer10(out_2)
